...
MotionModel parseMotionModel(const std::string & motion_model)
{
  ...
  throw std::runtime_error("MppiController: unknown motion_model '" + motion_model +
...
{
  if (!condition) {
    throw std::runtime_error("MppiController parameter validation failed: '" + name + "' " +
                             rule);
  }
}
...
bool MppiController::updateParamsFromNode(
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  if (!node) {
    throw std::runtime_error("MppiController: parent node expired in configure()");
  }
  ...
  RCLCPP_INFO(
    logger_,
    "Configured MppiController '%s': K=%d, T=%d, dt=%.3f (GPU rollouts)",
...
MppiController::DiagnosticsCsv MppiController::openDiagnosticsCsv(
  const std::string & path) const
{
  ...
  if (!output.file.is_open()) {
    throw std::runtime_error(
            "MppiController: failed to open diagnostics_csv_path '" + path + "'");
  }
  output.enabled = true;
  output.file
...
void MppiController::emitDiagnostics(
  ...
  int costmap_size_x, int costmap_size_y)
{
  ...
  if (diagnostics_log_period_ > 0.0 && node) {
    ...
    if (due) {
      ...
      RCLCPP_INFO(
        logger_,
        "MPPI diagnostics: solve=%.2f ms valid=%d/%d (%.1f%%) best=%.3f mean=%.3f "
...
std::vector<float> MppiController::extractLocalPath(
  const geometry_msgs::msg::PoseStamped & robot_pose,
  float & goal_x, float & goal_y, float & goal_yaw, bool & goal_is_final)
{
  if (global_plan_.poses.empty()) {
    throw ControllerInvalidPath("MppiController: received an empty plan");
  }
  ...
  try {
    ...
    throw ControllerTFError(
            std::string("MppiController: failed to transform plan: ") + ex.what());
  }
...
{
  if (!optimizer_) {
    throw ControllerException("MppiController is not configured");
  }
  ...
  if (result.all_colliding && !result.retreating) {
    throw NoValidControl(
            "MppiController: all sampled trajectories are in collision");
  }
